...
class BashWrapperBuilderTest extends Specification {
    ...
    def 'test bash wrapper with docker mount' () {
        ...
        folder.resolve('.command.run').text ==
                ...
                tee2=\$!
                (
                docker run -i -v /folder\\ with\\ blanks:/folder\\ with\\ blanks:ro -v ${folder}:${folder} -v "\$PWD":"\$PWD" -w "\$PWD" --entrypoint /bin/bash --name \$NXF_BOXID busybox -c "/bin/bash -ue ${folder}/.command.sh"
                ) >"\$COUT" 2>"\$CERR" &
                pid=\$!
                wait \$pid || ret=\$?
                wait \$tee1 \$tee2
